...
export type ScriptType = 'rules' | 'db'

/** Generate the code of the Auth0 script as a string */
export async function generateScript(
  ...
  const filedata = await fs.readFile(
    path.join(__dirname, '../scripts', scriptType, 'src', `${file}.js`)
  )
  const scriptBase = [
...
/**
 * Compare two scripts and print the diff between them
 *
 * Takes scriptType as a convenience parameter to enable nicer printing, but is
...
>(diffs: [T, Change[]][], scriptType: ScriptType) {
  ...
  for (const [scriptDef, changes] of diffs) {
    ...
    if (!changesHeaderPrinted) {
      console.log(
        `[[ Changed ${scriptType === 'rules' ? 'rules' : 'db scripts'} ]]`
      )
